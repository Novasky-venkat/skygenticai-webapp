...
1. 💎 **Hallmark Skill Set** (`skills/hallmark/`): UI Craft, Canvas-First, 3-Tier Token Inheritance, High-Fidelity Glossy Controls.
2. 🔦 **Lighthouse Skill Set** (`skills/lighthouse/`): WCAG 2.1/2.2 AA Accessibility (a11y), Screen-Reader Landmarks, ARIA Tree, Focus Rings.
3. 🔍 **SEO Skill Set** (`skills/seo/`): Schema.org JSON-LD Structured Data, OpenGraph, Canonical URLs, Sitemap & Robots.
...
### Execution Workflow:
1. **Phase 1 (Research)**: Review existing tokens in `styles/tokens.css`. Never invent magic hex colors.
2. **Phase 2 (Canvas Specification)**: Draft layout bounds (`1350px`/`1130px`) and the **6 Component States** (*Idle, Hover, Focus-Visible, Active, Disabled, Loading*) in `docs/canvas-specs.md`.
...
4. **Phase 4 (Hallmark Implementation)**: Scaffold modular components in `components/` using Tier-2 semantic tokens and Tier-3 variant modifiers.
